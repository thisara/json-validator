...
import com.thisara.dao.MetaDAO;
...
public class CarServiceImpl implements CarService {
	...
	public void save(PostMetaDTO postCarDTO) throws ServiceException {
		
		try {
			
			Car car = modelMapper.map(postCarDTO, Car.class);
			
			car = (Car)RecordAuditor.createAuditFields(car, postCarDTO.getUser().getName());
			
			Engine engine = modelMapper.map(postCarDTO, Engine.class);

			engine = (Engine)RecordAuditor.createAuditFields(engine, postCarDTO.getUser().getName());
			...
			for (Tyre tyre : tyres) {
				...
				tyre = (Tyre)RecordAuditor.createAuditFields(tyre, postCarDTO.getUser().getName());
